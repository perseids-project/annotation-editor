...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="tei:w">
        ...
        <!-- add spaces back -->
        <xsl:if test="local-name(following-sibling::*[1]) = 'w' or local-name(parent::*[1]) = 'seg' or local-name(parent::*[1]) = 'hi'">
            <xsl:text> </xsl:text>
        </xsl:if>
    </xsl:template>
    ...
    <xsl:template match="w">
        ...
        <!-- add spaces back -->
        <xsl:if test="following-sibling::w or parent::seg or parent::hi">
            <xsl:text> </xsl:text>
        </xsl:if>
    </xsl:template>
